...
class ImovelVisualizarView:
    ...
    async def handle_visualizar_imovel(self, http_request: HttpRequest, count_view: bool = True) -> HttpResponse:
        try:
            imovel_id = http_request.param.get("imovel_id")
            body_response = await self.__controller.visualizar(imovel_id, count_view)
