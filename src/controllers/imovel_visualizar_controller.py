...
class ImovelVisualizarController(ImovelVisualizarControllerInterface):
    ...
    async def visualizar(self, imovel_id: int, count_view: bool) -> Dict:
        imovel = await self.__busca_imovel_db(imovel_id, count_view)
        response = self.__format_response(imovel)
        return response

    async def __busca_imovel_db(self, imovel_id: int, count_view: bool) -> Imovel:
        imovel = await self.__imovel_repository.visualizar_imoveis(imovel_id, count_view)
        return imovel
